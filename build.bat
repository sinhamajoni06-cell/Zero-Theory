...
:: Library Path (-L)
set LIBS=-L %MSYS2_ROOT%/ucrt64/lib -lsfml-graphics -lsfml-window -lsfml-system -lopengl32 -lwinmm -lgdi32 -lfreetype -lstdc++ -lcomdlg32
...
if /i "%~1"=="MAP" (
    ...
    set "EXTRA_SOURCES=core/src/editor/zero_editor_session.cpp"
) else (
...
powershell -Command "$src = (Resolve-Path '%SOURCES%' | ForEach-Object { $_.Path }); $extra = '%EXTRA_SOURCES%'; if ($extra -ne '') { $src = $src + (Resolve-Path $extra.Split(' ') | ForEach-Object { $_.Path }) }; $allArgs = @('%CXX_STD%'.Split(' ')) + @('%FLAGS%'.Split(' ')) + @('%INCLUDES%'.Split(' ')) + @('-D_NOTDEBUG','%MAIN_FILE%') + $src + @('%LIBS%'.Split(' ')) + @('-o','%OUTPUT%'); $wd = (Get-Location).Path; $sw = [System.Diagnostics.Stopwatch]::StartNew(); $job = Start-Job -ScriptBlock { param($exe,$jargs,$dir) Set-Location $dir; & $exe @jargs 2>&1 } -ArgumentList '%COMPILER%', $allArgs, $wd; $pct=0; while ($job.State -eq 'Running') { if ($pct -lt 90) { $pct += 3 } elseif ($pct -lt 99) { $pct++ }; if ($pct -gt 99) { $pct = 99 }; $filled = [Math]::Floor($pct/5); $bar = ('#' * $filled) + ('-' * (20-$filled)); Write-Host -NoNewline \"`r[\"; Write-Host -NoNewline -ForegroundColor Yellow \"BUILDING\"; Write-Host -NoNewline \"] [ $bar ] $pct%%   \"; Start-Sleep -Milliseconds ([Math]::Max(30, 150 - $pct)) }; $result = Receive-Job -Job $job -Wait; Remove-Job -Job $job; $sw.Stop(); $elapsed = ($sw.Elapsed.TotalSeconds).ToString('0.00'); if ($job.State -eq 'Completed' -and ($result -join \"`n\") -notmatch 'error:') { Write-Host -NoNewline \"`r[\"; Write-Host -NoNewline -ForegroundColor Green \"FINISHED\"; Write-Host \"] [ #################### ] 100%% - Done in ${elapsed}s   \" } else { Write-Host -NoNewline \"`r[\"; Write-Host -NoNewline -ForegroundColor Red \"FAILED\"; Write-Host \"] Compilation failed!                  \"; $result | ForEach-Object { Write-Host $_ }; exit 1 }"
